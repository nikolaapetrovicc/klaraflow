interface BottomNavProps {
  currentScreen: 'dashboard' | 'log' | 'insights' | 'history' | 'calendar';
  onScreenChange: (screen: 'dashboard' | 'log' | 'insights' | 'history' | 'calendar') => void;
}
  ...
  const navItems = [
    {
      ...
      label: 'Home',
      icon: (active: boolean) => (
        <svg className={`w-6 h-6 ${active ? 'text-[#C27CA3]' : 'text-[#867B9F]'}`} fill="currentColor" viewBox="0 0 24 24">
          ...
          return (
            ...
            >
              ...
              <span className={`
                ...
                ${isActive ? 'text-[#C27CA3]' : 'text-[#867B9F]'}
              `}>
                {item.label}
              </span>
            </button>
